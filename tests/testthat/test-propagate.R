...
test_that("test of the propagate wrapper for multiple period", {
  expect_equal(propagate_flag(f,"weighted",flag_weights=w1),list(date1=list("s","1"),date2=list(as.character(NA),as.character(NA))))
...
test_that("test of the propagate wrapper for single period", {
  expect_equal(propagate_flag(fs,"weighted",flag_weights=ws1),list(date1=list("s","1")))
  ...
  expect_equal(propagate_flag(f[,1],"weighted",flag_weights=w1[,1]),list(date0=list("s","1")))
